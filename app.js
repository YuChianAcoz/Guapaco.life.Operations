...
// ===== 點陣機 Excel 匯出 =====
// 使用業主提供的 Excel 版型做為「位置模板」。下載檔只保留 A:S 的列印區，
// 不會把原本 Excel 右側的車輛/料單資料庫一起輸出。
// 內嵌點陣機 Excel 模板：直接雙擊 index.html 也可使用，不需本機伺服器。
...
function keepPrintAreaOnly(templateSheet) {
  const ws = structuredClone(templateSheet);

  Object.keys(ws).forEach((key) => {
    if (key.startsWith("!")) return;
    const cell = XLSX.utils.decode_cell(key);
    // A:S = 0..18；第 1~12 列 = 0..11
    if (cell.c > 18 || cell.r > 11) delete ws[key];
  });

  ws["!ref"] = "A1:S12";
  if (Array.isArray(ws["!cols"])) ws["!cols"] = ws["!cols"].slice(0, 19);
  if (Array.isArray(ws["!rows"])) ws["!rows"] = ws["!rows"].slice(0, 12);
  if (Array.isArray(ws["!merges"])) {
    ws["!merges"] = ws["!merges"].filter(
      (m) => m.s.c <= 18 && m.e.c <= 18 && m.s.r <= 11 && m.e.r <= 11,
    );
  }
...
function writeTemplateCell(ws, address, value, extraStyle = null) {
  const old = ws[address] || {};
  const cell = { ...old };
  delete cell.f;
  delete cell.F;
  delete cell.w;
...
function fillPrinterSheet(ws, r) {
  // 依廠商提供的 LQ-690CII 專用檔位置直接寫入，不再依賴右側資料庫/VLOOKUP。
  writeTemplateCell(ws, "N1", `NO:${r.deliveryNo || ""}`);
  writeTemplateCell(ws, "E3", r.customer || "");
  writeTemplateCell(ws, "L3", num(r.grossWeight));
  writeTemplateCell(ws, "E4", r.location || "");
  writeTemplateCell(ws, "L4", num(r.tareWeight));
  writeTemplateCell(ws, "E5", r.driver || "");
  writeTemplateCell(ws, "L5", num(r.netWeight));
  writeTemplateCell(ws, "E6", r.vehicleNo || "");
  writeTemplateCell(ws, "M6", r.productName || "");
  writeTemplateCell(ws, "E7", rocDate(r.date));
  writeTemplateCell(ws, "H7", r.departureTime || "");
  writeTemplateCell(ws, "J7", r.oilLarge === "" ? 0 : r.oilLarge || 0);
  writeTemplateCell(ws, "O7", r.oilSmall === "" ? 0 : r.oilSmall || 0);
  writeTemplateCell(ws, "R9", printerDocumentText(r.documentType));
  writeTemplateCell(ws, "R10", `第${r.tripNo || ""}車次`);
  writeTemplateCell(ws, "R11", "共");
  writeTemplateCell(ws, "R12", `${ft(r.cumulativeTons)}噸`);

  // 廠商原檔的「調度」是文字方塊；SheetJS 匯出時改用儲存格文字，避免固定舊姓名被帶出。
  const dispatcher = String(r.dispatcher || "").trim();
  if (dispatcher) {
    writeTemplateCell(ws, "E8", [...dispatcher].join("\n"), {
      alignment: { horizontal: "center", vertical: "center", wrapText: true },
    });
  } else {
    writeTemplateCell(ws, "E8", "");
  }
}
...
async function exportPrinterWorkbook() {
  ...
  try {
    const templateWb = XLSX.read(PRINTER_TEMPLATE_BASE64, {
      type: "base64",
      cellStyles: true,
      cellDates: false,
      bookFiles: false,
    ...
    if (!templateSheet) throw new Error("找不到點陣機 Excel 模板工作表");

    // 建立全新的活頁簿，只放列印用工作表；不帶原檔右側資料庫，也不帶第二張原始資料表。
    const outWb = XLSX.utils.book_new();
    state.records.forEach((r, index) => {
      const ws = keepPrintAreaOnly(templateSheet);
      fillPrinterSheet(ws, r);
      const sheetName = `車次${String(index + 1).padStart(2, "0")}`;
      XLSX.utils.book_append_sheet(outWb, ws, sheetName);
    });

    // 每張工作表的列印範圍固定 A1:S12，對應廠商原始版型。
    outWb.Workbook = outWb.Workbook || {};
    outWb.Workbook.Names = outWb.Workbook.Names || [];
    outWb.SheetNames.forEach((sheetName, i) => {
      outWb.Workbook.Names.push({
        Name: "_xlnm.Print_Area",
        Sheet: i,
        Ref: `'${sheetName.replaceAll("'", "''")}'!$A$1:$S$12`,
      });
    });

    const batchDate = (state.records[0]?.date || el("date").value || today).replaceAll("-", "");
    XLSX.writeFile(outWb, `點陣機料單_${batchDate}.xlsx`, {
      cellStyles: true,
      compression: true,
    });
    toast(`已下載點陣機檔案，共 ${state.records.length} 車次`);
  } catch (err) {
    console.error(err);
    alert(
      "點陣機檔案產生失敗。\n\n" +
