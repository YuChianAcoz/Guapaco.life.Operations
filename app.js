...
function buildRecordPlan() {
  ...
  while (index < 10000) {
    ...
    if (minPossible > batchTargetKg) {
      showMessage(
        `無法在淨重 ${fi(s.netMin)}～${fi(s.netMax)} kg（生成值以 10 kg 為單位）的設定範圍內，精準組成 ${ft(s.targetTons)} 公噸。請調整目標重量、淨重範圍或車輛空車重。`,
      );
      return null;
    }
  ...
  // 分配不重複淨重；以 10 kg 為單位，前後車優先相差 200～300 kg，最後一車排最小值。
  ...
  if (!calculatedNetWeights) {
    showMessage(
      `無法在淨重 ${fi(s.netMin)}～${fi(s.netMax)} kg 的範圍內，同時達成「每台淨重不重複、以 10 kg 為單位、累計淨重精準吻合」。請放寬淨重範圍或調整目標重量。`,
    );
    return null;
  }
